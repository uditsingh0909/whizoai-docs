...
const mapResponse = await fetch('https://api.whizoai.com/v1/map', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY',
...
const checkResults = async () => {
  const resultResponse = await fetch(`https://api.whizoai.com/v1/map/${jobId}`, {
    headers: {
      'Authorization': 'Bearer YOUR_API_KEY'
    }
  });
...
while True:
    result_response = requests.get(
        f'https://api.whizoai.com/v1/map/{job_id}',
...
// Start mapping
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://api.whizoai.com/v1/map');
...
// Poll for results
while (true) {
    curl_setopt($ch, CURLOPT_URL, "https://api.whizoai.com/v1/map/{$jobId}");
...
curl -X GET "https://api.whizoai.com/v1/map/{jobId}" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
