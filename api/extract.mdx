...
const extractResponse = await fetch('https://api.whizoai.com/v1/extract', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY',
...
response = requests.post(
    'https://api.whizoai.com/v1/extract',
    headers={
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json'
    },
...
curl_setopt($ch, CURLOPT_URL, 'https://api.whizoai.com/v1/extract');
...
curl -X POST "https://api.whizoai.com/v1/extract/schema" \
...
# Batch extraction
curl -X POST "https://api.whizoai.com/v1/extract/batch" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "urls": [
